# emerging-technologies-certification
Certificate of Completion – Emerging Technologies Training (Edunet Foundation | SAP | Code Unnati)
# 🎓 Emerging Technologies Certification

This repository contains my **Certificate of Completion** for successfully completing an **Advanced Course on Emerging Technologies**, conducted under the **Code Unnati Program**, a CSR initiative by **SAP**, and implemented by **Edunet Foundation**.

---

## 📜 Certificate Details

- **Candidate Name:** Mergu Rithika  
- **Institution:** Avanthi Institute of Engineering & Technology  
- **Program:** Code Unnati – Emerging Technologies  
- **Duration:** 2024 – 2025  
- **Supported By:** SAP  
- **Implemented By:** Edunet Foundation  
- **Certificate ID:** CU25_26141  

---

## 🧠 Technologies Covered

The training focused on the following cutting-edge technologies:

- Machine Learning  
- Deep Learning  
- Internet of Things (IoT)  
- Computer Vision  
- SAP ABAP  
- SAP Business Technology Platform (BTP)  

These technologies are essential for modern software development, data science, and enterprise applications.

---

## 🖼 Certificate

![Emerging Technologies Certificate](certificate.png)

> This certificate validates my participation and successful completion of the training program.

---

## 🚀 Key Outcomes

- Practical exposure to emerging technologies  
- Hands-on experience with industry-relevant tools  
- Strong foundation in AI and enterprise solutions  
- Industry-recognized certification supported by SAP  

---

## 🏫 Issuing Authorities

- **Edunet Foundation**  
- **Code Unnati Program**  
- **SAP (CSR Initiative)**  
- **TASK – Telangana Academy for Skill & Knowledge**

---

## 👩‍💻 Author

**Mergu Rithika**  
Aspiring Data Scientist | Machine Learning & AI Enthusiast  

---

## 🔗 References

- Edunet Foundation: https://edunetfoundation.org  
- SAP CSR Initiatives: https://www.sap.com  
- TASK: https://task.telangana.gov.in
